    ...
    $(document).ready(function () {
        ...
        $("#toFound").submit(function (e) {
            ...
            $.ajax({
                ...
                type: "POST",
                data: {
                    id_people: $("#searchPeopleId").val(),
                    date_financial_release: $("#searchDate").val(),
                    type_mov: $("#typeSearch").val()
                },
                success: function (data) {
                    var saida = 0;
                    var entrada = 0;
...
<div>
    ...
    <div class="container">
        ...
        <!-- Div com valores positivos e negativos -->
        <div class="card-panel row center">
            <div class="col s6" id="tEntrada">
                <h4 class="green-text">Total entrada: R$ <?= number_format($totalPositive, 2, ',', '.'); ?></h4>
            </div>
            <div class="col s6" id="tSaida">
                <?php if ($totalNegative == 0) { ?>
                    <h4 class="red-text">Total saída: R$ <?= number_format($totalNegative, 2, ',', '.'); ?></h4>
                <?php } else { ?>
                    <h4 class="red-text">Total saída: R$ -<?= number_format($totalNegative, 2, ',', '.'); ?></h4>
                <?php } ?>
            </div>
                ...
                <table class="bordered highlight">
                    ...
                    <?php foreach ($movimentations as $movimentation) : ?>
                        <tr>
                            ...
                            <td style="text-align: center;"><?= $movimentation['type_mov'] ?></td>
                            <td>
                                <a href="update-movimentation-form/<?= $movimentation['id_financial_release'] ?>"><i
                                        class="material-icons">edit</i></a>
                                <a href="#deleteMovModal" id="<?= $movimentation['id_financial_release'] ?>"
                                   class="deleteMovModal"><i class="material-icons">delete</i></a>
                            </td>
                        </tr>
                    <?php endforeach ?>
                    </tbody>
        ...
        <div class="modal-content">
            <form action="delete-movimentation" id="delete-c" method="POST">
                ...
                <div class="modal-footer">
                    ...
                    <a href="#!" onClick="document.getElementById('delete-c').submit();"
                       class="waves-effect waves-red btn red">Apagar</a>
                </div>
            </form>
        </div>
    </div>
